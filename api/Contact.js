...
// Fallback mémoire si Supabase non configuré ou en erreur
...
let mem = []; // fallback mémoire serveur
...
export default async function handler(req, res) {
  ...
  if (!hasSupabase) {
    if (req.method === "GET") return res.status(200).json({
      items: mem, source: "memory",
      message: "Supabase non configuré - mode mémoire"
    });
    if (req.method === "POST") {
      try {
