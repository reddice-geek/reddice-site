// /api/contact.js - V3.1 100% fonctionnel
...
let mem=[]; // fallback mémoire si pas de Supabase

export default async function handler(req,res){
  // CORS pour test local
  ...
  if(!hasSupabase){
    if(req.method==="GET") return res.status(200).json({items:mem, source:"memory", message:"Supabase non configuré - mode mémoire"});
    if(req.method==="POST"){
      try{
        ...
        if(!name||!email||!message) return res.status(400).json({error:"Champs requis: name, email, message"});
      ...
      }catch(e){ return res.status(500).json({error:"contact memory error", details:e.message})}
    }
  ...
  try{
    if(req.method==="GET"){
      ...
      if(!Array.isArray(items)) return res.status(200).json({items:mem, source:"memory_fallback", raw:text});
    ...
    if(req.method==="POST"){
      ...
      let rows;
      try{ rows=JSON.parse(rowsText); }catch{ rows=[]; }
      if(!r.ok) {
        // fallback mémoire si insert fail
        const item={id:"mem_"+Date.now(), name, email, subject, message, created_at:new Date().toISOString(), supabase_error:rowsText};
        mem.unshift(item);
        return res.status(200).json({ok:true, item, source:"memory_fallback", warning:"Supabase insert failed, saved in memory", supabase_response:rowsText});
      }
